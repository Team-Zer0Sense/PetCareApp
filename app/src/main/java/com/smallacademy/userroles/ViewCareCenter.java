...
public class ViewCareCenter extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_care_center);
        ...
        //query
        Query query = fStore.collection("CareCenter");
        //recyle option
        FirestoreRecyclerOptions<CareCenterModel> options = new FirestoreRecyclerOptions.Builder<CareCenterModel>()
                .setQuery(query, new SnapshotParser<CareCenterModel>() {
                    ...
                    public CareCenterModel parseSnapshot(@NonNull DocumentSnapshot snapshot) {
                        return new CareCenterModel(snapshot.getId(),snapshot.getString("CenterName"),snapshot.getString("FirstName"), snapshot.getString("LastName"), snapshot.getString("Mobile"), snapshot.getString("Email"), snapshot.getString("Address"));
                    }
                })
                .build();

        adapter = new FirestoreRecyclerAdapter<CareCenterModel, CareViewHolder>(options) {
            @NonNull
            @Override
            ...
            @Override
            protected void onBindViewHolder(@NonNull CareViewHolder holder, int position, @NonNull CareCenterModel model) {
                holder.model = model;
    ...
    private class CareViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
